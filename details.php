...
// Ensure the case ID is set and is a valid integer
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $caseId = (int)$_GET['id']; // Convert to integer to prevent SQL injection
} else {
    // Redirect or show an error if the ID is missing or invalid
...
if ($res == TRUE) {
    ...
    if ($count == 1) {
        ...
        // Redirect if no matching case is found
        header('location:' . SITEURL . 'cases.php');
        exit();
    }
} else {
